...
data class PersonAmountTransacted(
    var personAndBusiness: PersonAndBusiness,
    var amountTransacted: AmountTransacted
)

val transactionTypeArray = arrayOf<String>(
    "sentToNumber",
    "sentToBuyGoods",
    "sentToPayBill",
    "sentToMshwari",
    "receivedMoney",
    "receivedMoneyMshwari",
    "sentToBuyAirTime"
)
...
const val SENT_TO_NUMBER =
    """(?<code>\w+)\.*\s*Confirmed\.*\s*Ksh(?<amountSent>[\d\.\,]+) sent to (?<recipientName>[A-Z,a-z,\s]+)\s*(?<recipientNo>[0-9]+) on (?<date>\d{1,2}\/\d{1,2}\/\d{2}) at (?<time>\d{1,2}:\d{2} \w{2}).\s*New M-PESA balance is Ksh(?<balance>[\d\.\,]+)\.\s*Transaction cost, Ksh(?<transactionCost>[\d\.\,]+)\.\s*.*"""
const val SENT_TO_PAYBILL =
    """(?<code>\w+)\.*\s*Confirmed\.*\s*Ksh(?<amountSent>[\d\.\,]+) sent to (?<paybillNumber>[A-Z,a-z,\s]+)for account (?<accountNo>.+|)on (?<date>\d{1,2}\/\d{1,2}\/\d{2}) at (?<time>\d{1,2}:\d{2} \w{2}).\s*New M-PESA balance is Ksh(?<balance>[\d\.\,]+)\.\s*Transaction cost, Ksh(?<transactionCost>[\d\.\,]+)\.\s*.*"""
const val SENT_TO_MSHWARI =
    """(?<code>\w+)\.*\s*Confirmed\.*\s*Ksh(?<amountSent>[\d\.\,]+) transferred to M-Shwari account on (?<date>\d{1,2}\/\d{1,2}\/\d{2}) at (?<time>\d{1,2}:\d{2} \w{2}).\s*M-PESA balance is Ksh(?<mpesaBalance>[\d\.\,\s]+)\.\s*New M-Shwari saving account balance is Ksh(?<mshwariBalance>[\d\.\,\s]+)\.\s*Transaction cost Ksh\.(?<transactionCost>[\d\.\,]+)\.\s*.*"""
const val SENT_TO_BUY_GOODS =
    """(?<code>\w+)\.*\s*Confirmed\.*\s*Ksh(?<amountSent>[\d\.\,]+) paid to (?<recipient>.+) on (?<date>\d{1,2}\/\d{1,2}\/\d{2}) at (?<time>\d{1,2}:\d{2} \w{2})\.\s*New M-PESA balance is Ksh(?<balance>[\d\.\,]+)\.\s*Transaction cost, Ksh(?<transactionCost>[\d\.\,]+)\.\s*.*"""
const val RECEIVED_FROM_MSHWARI =
    """(?<code>\w+)\.*\s*Confirmed\.*\s*Ksh(?<amountSent>[\d\.\,]+) transferred from M-Shwari account on (?<date>\d{1,2}\/\d{1,2}\/\d{2}) at (?<time>\d{1,2}:\d{2} \w{2}).\s*M-Shwari balance is Ksh(?<mshwariBalance>[\d\.\,\s]+)\.\s*M-PESA balance is Ksh(?<mpesaBalance>[\d\.\,\s]+)\.\s*Transaction cost Ksh\.(?<transactionCost>[\d\.\,]+)\.\s*.*"""
const val ACCOUNT_BALANCE =
    """(?<code>\w+)\.*\s*Confirmed\.*\s*Your account balance was: M-PESA Account : Ksh(?<mpesaBalance>[\d\.\,]+) Business Account : Ksh(?<businessBalance>[\d\.\,]+) on (?<date>\d{1,2}\/\d{1,2}\/\d{2}) at (?<time>\d{1,2}:\d{2} \w{2})\.\s*Transaction cost, Ksh(?<transactionCost>[\d\.\,]+)\.\s*.*"""
const val SENT_TO_BUY_AIRTIME =
    """(?<code>\w+) Confirmed\.*\s*You bought Ksh(?<amountSent>[\d\.\,]+) of airtime on (?<date>\d{1,2}\/\d{1,2}\/\d{2}) at (?<time>\d{1,2}:\d{2} \w{2})\.*\s*New M-PESA balance is Ksh(?<balance>[\d\.\,]+)\.\s*Transaction cost, Ksh(?<transactionCost>[\d\.\,]+)\..*"""
const val RECEIVED_MONEY =
    """(?<code>\w+) Confirmed\.*\s*You have received Ksh(?<amountReceived>[\d\.\,]+) from (?<sender>.*) on (?<date>\d{1,2}\/\d{1,2}\/\d{2}) at (?<time>\d{1,2}:\d{2} \w{2})\.*\s*New M-PESA balance is Ksh(?<balance>[\d\.\,]+)\..*"""
...
//            val businessName = paybill + "*" + account
            ...
            val receipt = Receipt(
                message,
                code,
                transactionType = "sentToPayBill",
                date = convertDateToLong(date + " " + time),
                balance = convertToDouble(balance),
                amountSent = convertToDouble(amountSent),
                transactionCost = convertToDouble(transactionCost),
                name = person.name
            )
        ...
        sentToBuyGoods.matches(message) -> {
            ...
            val receipt = Receipt(
                message,
                code,
                transactionType = "sentToBuyGoods",
                date = convertDateToLong(date + " " + time),
                balance = convertToDouble(balance),
                amountSent = convertToDouble(amountSent),
                transactionCost = convertToDouble(transactionCost),
        ...
        sentToMshwari.matches(message) -> {
            ...
            val receipt = Receipt(
                message,
                code,
                transactionType = "sentToMshwari",
                date = convertDateToLong(date + " " + time),
                balance = convertToDouble(mpesaBalance),
                amountSent = convertToDouble(amountSent),
                transactionCost = convertToDouble(transactionCost),
                name = person.name
            )
        ...
        receivedMoney.matches(message) -> {
            ...
            time = formatTime(time)
            val person =
                PersonAndBusiness(sender) // e.g ALOICE  OLOO 0719249474, CITIBANK N.A KENYA 100200
            val receipt = Receipt(
                message, code, transactionType = "receivedMoney",
        ...
        receivedMoneyMshwari.matches(message) -> {
            ...
            val person = PersonAndBusiness("MSHWARI")
            val receipt = Receipt(
                message, code, transactionType = "receivedMoneyMshwari",
                date = convertDateToLong(date + " " + time), balance = convertToDouble(balance),
                amountReceived = convertToDouble(amountReceived), name = person.name
            )
            return Pair(receipt, person)
        ...
        sentToBuyAirTime.matches(message) -> {
            ...
            val receipt = Receipt(
                message,
                code,
                transactionType = "sentToBuyAirTime",
                date = convertDateToLong(date + " " + time),
                balance = convertToDouble(balance),
                amountSent = convertToDouble(amountSent),
                transactionCost = convertToDouble(transactionCost),
...
private fun formatTime(date: String): String {
    Timber.i(Build.VERSION.SDK_INT.toString())
    when (Build.VERSION.SDK_INT) {
        in 1..23 -> {
            ...
            if (newstring.equals(date)) {
                ...
                Timber.i(newstring)
                return newstring //Return type for android 6
            }
        }
    }
    return date //Return type for android 11+
}
